...
4. [Размещаем проект на GitHub](#id13)  
  ...
  4.4 [Подключаем Github Pages для product-сборки](id#17)
...
Дальше необходимо снова открыть терминал и перейти в папку "dist", которую скомпилировал WebPack:
```cd ./dist```  
...
4. На данной вкладке ищем заголовок "Source" и заполняем:  
